...
// ✅ FIXED: Enhanced download function with proper monthwise and SHE chart handling
async function downloadChartAsEnlarged(chartId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // ✅ DEBUG: Log specific information for monthwise and SHE charts
      if (chartId.includes("monthwise")) {
        console.log("Monthwise Chart detected - Config:", {
          ...
          annotation: originalChart.config.options?.plugins?.annotation,
        });
      } else if (chartId.includes("she") || chartId.includes("SHE")) {
      ...
      // ✅ CRITICAL FIX: For monthwise chart, preserve dataset-specific datalabels
      if (chartId.includes("monthwise")) {
        // Ensure dataset-specific datalabels are preserved with proper font sizing
        if (enlargedConfig.data && enlargedConfig.data.datasets) {
          enlargedConfig.data.datasets.forEach((dataset) => {
        ...
        // For other charts, use global datalabels configuration
        if (
          enlargedConfig.options.plugins &&
              ...
              afterDatasetsDraw: function (chart) {
                ...
                data.datasets.forEach((ds, dsIndex) => {
                  ...
                  meta.data.forEach((point, i) => {
                    ...
                    // ✅ FIXED: Better handling for horizontal bar charts
      ...
      // ✅ FIXED: Special handling for monthwise and SHE charts
      const isMonthwiseChart = chartId.includes("monthwise");
      ...
      if (isMonthwiseChart) waitTime = 3000;
      // Extra long for monthwise
      else if (isSHEChart) waitTime = 2500;
      else if (isHorizontalBar) waitTime = 2000;
      ...
      // ✅ FALLBACK: Try html2canvas if available for complex charts
      if (
        (chartId.includes("monthwise") ||
          chartId.includes("she") ||
          chartId.includes("SHE")) &&
        typeof html2canvas !== "undefined"
      ) {
        console.log("Trying html2canvas fallback for complex chart");
        try {
...
// ✅ FIXED: Enhanced safe chart configuration cloning with proper monthwise and SHE chart handling
function safeCloneChartConfig(originalChart) {
  ...
  // ✅ FIXED: Enhanced dataset cloning with proper datalabels handling
  if (originalConfig.data && originalConfig.data.datasets) {
    originalConfig.data.datasets.forEach((originalDataset, index) => {
      ...
      // ✅ CRITICAL FIX: Preserve indexAxis for horizontal bar charts
      ...
      // ✅ CRITICAL FIX: Clone dataset-specific datalabels configuration
      if (originalDataset.datalabels) {
        dataset.datalabels = { ...originalDataset.datalabels };
      }
  ...
  // ✅ FIXED: Enhanced options cloning with proper scales configuration
  if (originalConfig.options) {
    ...
    if (originalConfig.options.plugins) {
      ...
      // ✅ CRITICAL FIX: Clone annotation plugin configuration
      if (originalConfig.options.plugins.annotation) {
        config.options.plugins.annotation = {
          ...originalConfig.options.plugins.annotation,
        };
      }
    }

    // ✅ CRITICAL FIX: Enhanced scales cloning
  ...
  // ✅ FIXED: Enhanced plugin preservation
  ...
  console.log("Enhanced cloned config for chart:", config);
  return config;
}
